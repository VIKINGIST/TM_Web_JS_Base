...
{
  alert(
    `Завдання 3: З клавіатури вводиться рік народження учня. Визначити вік учня.`
  );
  let now = new Date();
  ...
  let bdaynow = new Date(today.getFullYear(), bday.getMonth(), bday.getDate()); //ДН в поточному році
...
{
  alert(`Завдання 4: З клавіатури вводиться арендна плата за квартиру (за 1 місяць).\nВизначити скільки треба заплатити за 1 рік, за 2 роки та 10 років.
`);
  let sumPay = parseInt(
...
{
  alert(
    `Завдання 12: Скільки лампочок потрібно, щоб освітити вулицю довжиною D км, як що стовпи з ліхтарями стоять на відстані V м?`
  );
  let d = parseInt(prompt(`довжина вулиці`, `1.2 км.`));
  let v = parseInt(prompt(`відстань між ліхнатарями`, `10 м`));
  let lamps = Math.ceil((d * 1000) / v);
  alert(
